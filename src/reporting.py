    ...
    for cluster_id in list_cluster_id:
                ...
                for pokemon_entry in pokemons_list:
                    ...
                    if pokemon_name:
                        if pokemon_name not in pokemon_data:
                            pokemon_data[pokemon_name] = {"count": 0, "items": []}
                        pokemon_data[pokemon_name]["count"] += 1
                        # Add actual items for this Pokemon
                        if pokemon_items:
                            pokemon_data[pokemon_name]["items"].extend(pokemon_items)
        
        # Sort by frequency and keep top 10
        sorted_pokemons = sorted(pokemon_data.items(), key=lambda x: x[1]["count"], reverse=True)[:10]
        ...
        if sorted_pokemons:
            for pokemon, data in sorted_pokemons:
                ...
                if items_list:
                    ...
                    mean_items = len(items_list) / data["count"] if data["count"] > 0 else 0
                    # Cap at 3 since Pokemon can only hold 0-3 items
                    mean_items = min(mean_items, 3.0)
                    num_items = int(round(mean_items))  # Round to nearest integer
                    
                    # Count item frequencies
                    from collections import Counter
                    item_counts = Counter(items_list)
                    top_items = [item for item, _ in item_counts.most_common(num_items)]
                else:
                    mean_items = 0
                    top_items = []
        ...
        if len(points) >= 3:
            try:
                ...
                hull_points = [[float(points[i][0]), float(points[i][1])] for i in hull.vertices]
            except Exception as e:
                # If convex hull fails (e.g., colinear points), use a simple circle
                print(f"\tWarning: Could not compute convex hull for cluster {cluster_id}: {e}")
                hull_points = []
...
def export_meta_report_text(meta_report, output_dir):
    ...
    with open(filepath, 'w') as f:
        ...
        for cluster in meta_report:
            ...
            f.write(f"  Position: x={cluster.get('x', 0):.2f}, y={cluster.get('y', 0):.2f}\n")
            f.write("\n")
            ...
            if mean_team and "pokemons" in mean_team:
                ...
                for pokemon, data in pokemon_list.items():
                    ...
                    f.write(f"    - {pokemon}: {frequency:.1f}% frequency, {mean_items:.1f} avg items\n")
                    if items:
...
def visualize_meta_report(meta_report, output_dir):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filepath1 = os.path.join(output_dir, f"meta_clusters_positioning_{timestamp}.png")
    ...
    ax1.axhline(y=np.mean(winrates), color='r', linestyle='--', label='Average')
    ax1.legend()
    ...
    ax2.axhline(y=np.mean(mean_ranks), color='r', linestyle='--', label='Average')
    ax2.legend()
    ax2.invert_yaxis()
    ...
    if meta_report:
        ...
        if all_synergies:
            ...
            im = ax.imshow(np.array(synergy_matrix).T, cmap='YlOrRd', aspect='auto')
            ...
            plt.tight_layout()
            filepath3 = os.path.join(output_dir, f"meta_synergies_heatmap_{timestamp}.png")
