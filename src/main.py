...
from .data_loader import (
    load_data_mongodb, create_dataframe,
    create_item_data_elo_threshold, create_pokemon_data_elo_threshold,
    create_region_data
)
from .dimensionality import (
    apply_tsne, get_adaptive_perplexity, plot_tsne_parameters,
    plot_tsne_init_comparison, plot_tsne_metric_comparison,
...
def run_analysis(json_data, elo_threshold=None):
    ...
    if elo_threshold is not None:
        json_data_filtered = [m for m in json_data if m.get("elo", 0) >= elo_threshold]
        print(f"{datetime.now().time()} filtered to {len(json_data_filtered)} matches with elo >= {elo_threshold}")
    else:
        json_data_filtered = json_data
    
    if len(json_data_filtered) < 10:
        print(f"{datetime.now().time()} skipping 1100+ tier: insufficient data ({len(json_data_filtered)} matches)")
        return None
    ...
    # Testing flags
    SKIP_TSNE_COMPARISON = os.environ.get("SKIP_TSNE_COMPARISON", "true").lower() == "true"
    SKIP_DBSCAN_GRID = os.environ.get("SKIP_DBSCAN_GRID", "true").lower() == "true"
    ...
    adaptive_perplexity = get_adaptive_perplexity(n_samples, method="aggressive")
    adaptive_min_samples = get_adaptive_min_samples(n_samples, method="balanced")
    print(f"{datetime.now().time()} dataset size: {n_samples} samples, "
          f"adaptive perplexity: {adaptive_perplexity}, "
          f"adaptive min_samples: {adaptive_min_samples}")
    ...
    numeric_cols = [col for col in df_match.columns 
                    if col not in ["rank", "nbplayers", "pokemons", "items", "elo"] 
                    and pd.api.types.is_numeric_dtype(df_match[col])]
    
    if not numeric_cols:
        print(f"{datetime.now().time()} warning: no synergy columns found. Available columns: {list(df_match.columns)}")
        print(f"{datetime.now().time()} skipping t-SNE analysis")
        return None
    
    print(f"{datetime.now().time()} using {len(numeric_cols)} synergy columns for t-SNE: {numeric_cols[:5]}...")
    df_filtered = df_match[numeric_cols]
    
    df_tsne = apply_tsne(df_filtered, adaptive_perplexity, save=SAVE_PLOTS, output_dir=tsne_dir)
    
    if not SKIP_TSNE_COMPARISON:
        print(f"{datetime.now().time()} comparing t-SNE perplexity values...")
        perplexity_values = [
            ("balanced", get_adaptive_perplexity(n_samples, method="balanced")),
            ("conservative", get_adaptive_perplexity(n_samples, method="conservative")),
            ("aggressive", get_adaptive_perplexity(n_samples, method="aggressive")),
            ("log_based", get_adaptive_perplexity(n_samples, method="log_based")),
            ("high", 70),
            ("very_high", 90),
        ]
        perplexity_labels = [f"{method}={ppx}" for method, ppx in perplexity_values]
        perplexity_nums = [ppx for _, ppx in perplexity_values]
        print(f"{datetime.now().time()} testing perplexity values: {perplexity_labels}")
        ...
        plot_tsne_init_comparison(df_filtered, ["pca", "random"], output_dir=tsne_dir)
        
        print(f"{datetime.now().time()} comparing t-SNE distance metrics...")
        plot_tsne_metric_comparison(df_filtered, ["euclidean", "cosine", "manhattan"], output_dir=tsne_dir)
        
        print(f"{datetime.now().time()} comparing t-SNE learning rates...")
        plot_tsne_learning_rate_comparison(df_filtered, ["auto", 200, 500, 800], 
    ...
    if not SKIP_DBSCAN_GRID:
        ...
        min_samples_values = [
            get_adaptive_min_samples(n_samples, method="permissive"),
            get_adaptive_min_samples(n_samples, method="aggressive"),
            get_adaptive_min_samples(n_samples, method="balanced"),
            get_adaptive_min_samples(n_samples, method="conservative"),
        ]
        epsilon_values = [2, 2.5, 3, 3.5, 4]
    ...
    print(f"{datetime.now().time()} analysis complete for 1100+ tier! Results saved in: {results_dir}")
    return results_dir
