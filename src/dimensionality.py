"""t-SNE dimensionality reduction functions"""
...
def apply_tsne(df, perplexity, n_iter=None, plot=False, save=False, output_dir=None):
    ...
    if plot or save:
        ...
        plt.scatter(df_result["x"], df_result["y"], color="black", alpha=0.4, s=20)
        plt.title(f"t-SNE Visualization (perplexity={perplexity})")
        plt.xlabel("t-SNE Component 1")
        plt.ylabel("t-SNE Component 2")
        plt.grid(True, alpha=0.3)
        if save and output_dir:
            filepath = os.path.join(output_dir, f"tsne_perplexity_{perplexity}.png")
...
def get_adaptive_perplexity(n_samples, method="balanced"):
    """
    Calculate adaptive perplexity based on dataset size with optimized defaults for large datasets.
    
    Perplexity should be roughly between 5 and 50, but for large datasets (>100k),
    smaller values (20-40) often work better and compute faster.
    
    Args:
        n_samples: number of samples in dataset
        method: 'balanced' (default), 'conservative', 'aggressive', or 'log_based'
    
    Returns:
        Recommended perplexity value (integer)
    """
    if method == "conservative":
        return min(max(5, int(math.log(n_samples) * 2)), n_samples // 32)
    elif method == "aggressive":
        return min(max(50, int(math.log(n_samples) * 5)), n_samples // 3)
    elif method == "log_based":
        return min(max(10, int(math.log(n_samples) * 3.5)), n_samples // 3)
    else:  # balanced (default) - optimized for all dataset sizes
        if n_samples < 1000:
            return min(30, max(5, int(math.log(n_samples) * 2)))
        elif n_samples < 10000:
            return min(50, max(30, int(math.log(n_samples) * 3)))
        elif n_samples < 100000:
            return min(50, max(35, int(math.log(n_samples) * 3)))
        else:  # Large datasets: use smaller perplexity for speed
            return min(40, max(20, int(math.log(n_samples) * 2.5)))
...
def plot_tsne_parameters(df, list_perplexity, output_dir=None):
    ...
    for idx, ppx in enumerate(list_perplexity):
        ...
        sub_plt.scatter(df_tsne[0], df_tsne[1], color="black", alpha=.33, s=10)
...
def plot_tsne_init_comparison(df, list_inits, output_dir=None):
    ...
    for idx, init_method in enumerate(list_inits):
        ...
        ax_flat[idx].scatter(df_tsne[0], df_tsne[1], color="black", alpha=.33, s=10)
...
def plot_tsne_metric_comparison(df, list_metrics, output_dir=None):
    ...
    for idx, metric in enumerate(list_metrics):
        ...
        try:
            ...
            ax_flat[idx].text(0.5, 0.5, f"Failed: {metric}", ha='center', va='center')
...
def plot_tsne_learning_rate_comparison(df, list_learning_rates, adaptive_perplexity, output_dir=None):
    ...
    plt.tight_layout()
    if output_dir:
        filepath = os.path.join(output_dir, "tsne_learning_rate_comparison.png")
        plt.savefig(filepath, dpi=150, bbox_inches='tight')
...
def plot_tsne_early_exaggeration_comparison(df, list_early_exag, adaptive_perplexity, output_dir=None):
    ...
    plt.tight_layout()
    if output_dir:
        filepath = os.path.join(output_dir, "tsne_early_exaggeration_comparison.png")
        plt.savefig(filepath, dpi=150, bbox_inches='tight')
        print(f"{datetime.now().time()} saved early_exaggeration comparison to {filepath}")
        plt.close()
    else:
        plt.show()
