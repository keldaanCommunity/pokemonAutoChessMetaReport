...
def apply_kmeans_clustering(df, n_clusters, plot=False, save=False, output_dir=None):
    ...
    """
    df_result = df.copy()
    kmeans = KMeans(n_clusters=n_clusters, random_state=42, n_init=10).fit(df_result[["x", "y"]])
...
def get_adaptive_k_clusters(n_samples, method="balanced"):
    """
...
def plot_kmeans_parameters(df, list_k, output_dir=None):
    ...
    for idx, k in enumerate(list_k):
        ...
        df_cluster = df.copy()
        kmeans = KMeans(n_clusters=k, random_state=42, n_init=10).fit(df_cluster[["x", "y"]])
